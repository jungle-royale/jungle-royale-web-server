...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(DatabaseColumnMissingException.class)
    public ResponseEntity<Map<String, Object>> handleDatabaseColumnMissingException(DatabaseColumnMissingException e) {
        Map<String, Object> response = new HashMap<>();
        response.put("status", HttpStatus.INTERNAL_SERVER_ERROR.value());
        response.put("error", "DATABASE_COLUMN_MISSING");
        response.put("message", e.getMessage());
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
    }

    @ExceptionHandler(SQLGrammarException.class)
    public ResponseEntity<Map<String, Object>> handleSQLGrammarException(SQLGrammarException e) {
        String rootCauseMessage = e.getCause() != null ? e.getCause().getMessage() : e.getMessage();
        throw new DatabaseColumnMissingException("SQL error occurred: " + rootCauseMessage);
    }
